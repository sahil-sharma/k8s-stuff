...
# https://github.com/strimzi/strimzi-kafka-oauth/blob/0.17.1/examples/docker/kafka-oauth-strimzi/kafka/oauth.sh

# Usage: 
# export SSO_TOKEN_URL=http://sso.local.io:32080/realms/kafka-authz/protocol/openid-connect/token
# REFRESH_TOKEN=$(bash /opt/oauth.sh -q bob)
...
usage() {
  ...
  echo "   --endpoint  SSO_TOKEN_URL        Authorization server token endpoint"
...
if [ "$SSO_TOKEN_URL" == "" ]; then
    >&2 echo "SSO_TOKEN_URL not set. Use --endpoint with endpoint url to set it."
    exit 1
fi

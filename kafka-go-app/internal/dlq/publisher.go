...
// It reuses the same OAuth token as the main producer.
...
func NewPublisher(
	...
	p, err := kafka.NewProducer(&kafka.ConfigMap{
		...
		"sasl.oauthbearer.config": fmt.Sprintf("token=%s", initialToken),
...
// UpdateToken pushes a refreshed OAuth token to the DLQ producer.
func (d *Publisher) UpdateToken(token kafka.OAuthBearerToken) error {
	return d.producer.SetOAuthBearerToken(token)
}

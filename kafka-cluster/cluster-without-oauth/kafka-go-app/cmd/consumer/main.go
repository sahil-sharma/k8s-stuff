...
import (
	...
	// REMOVE: "kafka-go-app/internal/auth"
...
func main() {
	...
	// ── DLQ publisher (SCRAM — no token needed) ───────────────────────────────
	dlqPub, err := dlq.NewPublisher(bootstrapServers, dlqTopic, kafkaUsername, kafkaPassword, log)
	if err != nil {
		log.WithError(err).Fatal("failed to create DLQ publisher")
	}
	defer dlqPub.Close()

	// ── Confluent consumer (SCRAM replaces OAUTHBEARER) ───────────────────────
